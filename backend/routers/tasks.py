...
from helpers.auth.auth_utils import get_current_user, verify_current_user
from database import get_session
from models import Task, RecurringTask, User
...
def get_task(id: int, current_user = Depends(get_current_user), session: Session = Depends(get_session)):
...
def add_task(
    ...
    if repetitive_type or repeat_until:
        recurring_task = RecurringTask(task_id=task.id, repetitive_type=repetitive_type, repeat_until=repeat_until)
...
def update_task(id: int, current_user = Depends(get_current_user), session: Session = Depends(get_session)):
    ...

@router.delete("/delete-task/{id}")
def delete_task(id: int, current_user = Depends(get_current_user), session: Session = Depends(get_session)):
    task = get_task_by_id(id, session)
    msg = "Access denied"
    if verify_current_user(task.user_id, current_user["user_email"], session):
        session.delete(task)
